...
27. **Move general-purpose mask/label utilities to
    `autofrap/mask_utils.py`** (user TODOs in `detection.py`):
    `split_mask_along_axis_equal_area`, `shuffle_labels`,
    `relabel_by_distance`, `mask_to_polygon`; keep `cell_mask` +
    `half_object_stim_mask` in `detection.py` (domain glue, not
    general-purpose). Pure move, no behavior change; ripple: `autofrap.py`
    imports, bitsnpieces one-offs keep old names (the "pipeline copy is
    canonical" note then points at `mask_utils`). Also fix the naming pair
    (user TODO): `split_mask_along_axis_equal_area` → e.g.
    `split_mask_equal_area` ("along_axis" is redundant, axis is a
    parameter); `half_object_stim_mask` is fine (product name, TODO #18);
    docstring param `label_mask` vs arg `mask`.
