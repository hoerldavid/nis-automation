...
21. **Detection failure → always `NonRecoverableError`**: stop translating
    `requests` exception types in `autofrap()` (retry logic lives in
    `remote_detect_objects`; `autofrap.py`'s `import requests` exists only
    for those excepts). Consequence (deliberate, supersedes part of the
    TODO #9 decision): a per-image 5xx after retries now aborts the grid
    instead of skipping the FOV — a 5xx means the server itself is likely
    broken, so it would probably fail on the next FOV anyway; the
    Recoverable/NonRecoverable split then only covers NIS-side, FOV-local
    failures (no polygon, ROI creation). Update docstrings + TODO #9 note.
22. **Allow a bare label map (ndarray) as `detection_fun` return** instead
    of a 1-tuple (one line: `isinstance(det, np.ndarray)` → `(det,)`);
    update the contract docstrings (`autofrap.py`, `detection.py` module
    docstring, `DESIGN_GOALS_AUTOFRAP.md` step 6).
