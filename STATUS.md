...
_Last updated: **`mask_utils.py` split out of `detection.py` (TODO #27)** (no microscope): the user moved the general-purpose mask/label utilities into `autofrap/mask_utils.py` (`split_mask_along_axis_equal_area` → **renamed `split_mask_equal_area`** — "along_axis" was redundant, axis is a parameter; docstring param `label_mask` → `mask`; `half_object_stim_mask` moved with them since it wraps the split, `_half_object_stim_mask` adapter in `autofrap.py` follows; `cell_mask` stays in `detection.py` — domain glue, not general-purpose). The review found the move broken at every entry point, now fixed: `mask_utils.py` was missing `import numpy as np`, and the new imports used package-qualified `autofrap.mask_utils` / `from autofrap import ...` style — that resolves in the user's IDE (autofrap/ as a source root, where `autofrap` is the `autofrap.py` module) but fails in every runtime context (no `__init__.py`, bitsnpieces tests add only `autofrap/` to `sys.path`; `autofrap/autofrap.py` even self-shadowed as a module candidate). Everything now uses the codebase's plain style (`import mask_utils`, `from mask_utils import ...` — `autofrap/` is on `sys.path` in all contexts). The dangling one-region-per-label TODO comment was relocated next to `half_object_stim_mask` (its semantic home; text unchanged). Also fixed a **pre-existing** bug found while verifying: `detection.py` `__main__` unpacked `labels, stim =` but passes `visualization_fun` → 3-tuple (broken since the viz commit; `__main__` just was never re-run). Tested: `detection.py` `__main__` on a real survey file, `test_detect_viz` + `test_autofrap_detection_contract` + `test_read_channel` all pass, `autofrap.py --help` OK; `test_qc_overlay.py` (references updated by the user, import fixed) compiles but needs the git-ignored `0013_ch1.tif` to run. New **TODO #30** (user): clean module/package structure for the final version — no `sys.path` manipulation, `from autofrap import next_stimulatable_cell`-style imports made possible by IDE source-root settings, etc. Previous: **`autofrap()` detection contract simplified (TODOs #21 + #22, from the user's `autofrap.py` TODOs)** (no microscope): any `detection_fun` failure now → `NonRecoverableError` — the three `requests` exception translations are gone (the retry logic lives in `remote_detect_objects`; a server that 5xxes after retries is suspect for the next FOV as well, so a grid run aborts — deliberately supersedes part of the TODO #9 decision, where a per-image 5xx used to skip the FOV and continue). `autofrap.py` no longer imports `requests` (its only use was those excepts — the module now imports cleanly without it). A **bare label map (ndarray)** is an accepted `detection_fun` return (normalized to a 1-tuple before the 1–3 tuple/list check; `build_detector` still always returns tuples — this serves hand-rolled detectors passed straight to `autofrap()`). Docstrings updated: module docstring step 2, `autofrap()` `detection_fun` parameter + `Raises` section, both error-class docstrings, `detection.py` module docstring; `DESIGN_GOALS_AUTOFRAP.md` left untouched (step 6 already lists the label map as the only non-optional return). Test: `autofrap_bitsnpieces/test_autofrap_detection_contract.py` (12 checks, faked nis_util + empty placeholder survey file the fakes never read: bare/1-3-tuple/list returns accepted, 4-tuple + str rejected, ConnectionError/Timeout/HTTPError/RuntimeError all → NonRecoverableError, no `requests` attribute on the module). Note: `requests` was missing from this container (it is in `requirements.txt`) — installed; `test_autofrap_errors.py` is now further stale (it expected the old per-image-5xx → RecoverableError; left as-is per convention). Previous: **`read_channel` rewrite — axis-order independent, multi-channel, dimension validation, opt-in Z projection (TODO #19, from the user's `nd2_helpers.py` TODOs)** (no microscope): `f.asarray()[channel]` assumed channel-first axis order — a **real bug**: `01.nd2` (Z-stack the user copied to the repo root; 7×2×1608², untracked + not git-ignored, candidate for `test_acquisitions/`) is `(Z, C, Y, X)`, where the old code returned a 3D `(c, y, x)` array. New implementation validates the dimensions via `f.sizes` (dict axis-name → size, in array axis order) and enforces the **survey-only contract**: C/Y/X (plus Z when a projection is requested); anything else (T, P, …) raises `ValueError` — FRAP timeseries belong to downstream analysis, not the acquisition pipeline (user). Channel selection is axis-order independent (`np.transpose` to (C, Z, Y, X) as present; no xarray — not in `requirements.txt`, and nd2 0.11.3 has no writer to synthesize test files, so the array logic is factored out as `nd2_helpers._extract` for testing). `channel` now takes an int → 2D (y, x) (old behavior, value-identical on all 18 channels of the CYX survey files) or a tuple of ints → (k, y, x) in the given order, so the multi-channel `load_fun` from the `build_detector` docstring is one file read instead of two. New `z_projection='max'` (opt-in; the default `None` **errors** on a Z file so an accidentally Z-stacked survey can't be silently projected). NIS omits the C dimension of single-channel files (the TYX FRAP files have none): `channel=0` → 2D for those. Test: `autofrap_bitsnpieces/test_read_channel.py` (24 checks: old-behavior equivalence over the copied runs, T rejection on a TYX FRAP file, Z-stack default/max/ordering, synthetic axis permutations incl. no-C + odd orders — one real `_extract` bug found on the way: single-channel + Z projected the wrong axis); `test_detect_viz.py` still passes; the default `build_detector` composition runs on a real survey file. Data/docs: the user copied one test auto-FRAP run (20260824, 3×2 cycles, dummy detector) into `test_acquisitions/autofrap_out/` (file map already described it) and the NIS macro reference `nis_ar_help_html/` (3717 pages, git-ignored) is now in the workspace — which settled several open questions: `StgMoveXY` is documented blocking-style (DR_OK = "XY device moved to specified position", no async marker, unlike the explicitly-async fns), **`ActivateDocument(name)`** makes an already-open document current without re-loading from disk, and `ND_RunExperiment(1)` = "the resulting file is opened after the experiment finishes" (see TODOs #24–#26). The user's remaining TODO batch is registered as #20–#29. Previous: **`visualization_fun` for `build_detector()` (TODO #18, first half) + renames/conventions** (no microscope): `detect()` renamed to **`build_detector()`** (it builds, not detects), `default_stimulation_mask` renamed to **`half_object_stim_mask`** (the name now says what it does; the `autofrap.py` adapter follows as `_half_object_stim_mask`), and the multi-channel image convention is now **(c, y, x)** (scientific format) everywhere — `load_fun` output, `detector_fun` input, `dummy_detect_objects` (uses `shape[-2:]`) — with one deliberate exception: the visualization is **(y, x, 3/4)** RGB(A), the display format `qc.save_qc_overlay` expects (a (c, y, x) load becomes an RGB viz via a transpose, e.g. `lambda image: np.transpose(image, (1, 2, 0))`). New part `visualization_fun(image)` → 2D (y, x) or (y, x, 3/4) RGB(A) image for the QC overlay. Contract: it receives the *loaded image* (2D or multi-channel — the same one the detector gets), not the labels (the viz is a property of the image, not of the detection result); `None` (default) → no viz (blank canvas, previous behavior). The return tuple keeps its fixed positions (2 = mask, 3 = viz): viz without mask → `(labels, None, viz)`. Best effort: a failing or malformed viz (exception, or a wrong shape, e.g. a (y, x, 2) image) only **warns and drops the viz** — it is cosmetic and must not break the run (contrast: the labels/mask contract checks still raise `ValueError`). `autofrap()`'s default + the CLI now pass `visualization_fun=lambda image: image` — the QC overlay shows the ch0 DAPI instead of a black canvas (free: it is the same image detection already loads). Also fixed a latent bug: with `stim_mask_fun=None`, `build_detector()` returned a **bare** `labels` array, but `autofrap()` only accepts a 1–3 *tuple* → now returns `(labels,)` (its own docstring already said `(labels,)`); any no-mask composition would previously have crashed `autofrap()`. `dummy_detect_objects` now takes (y, x, c) too (uses only `shape[:2]`), matching the detector contract; `detection.py` `__main__` updated to the new API. Tested in-container with `autofrap_bitsnpieces/test_detect_viz.py` (synthetic, 19 checks: all four return-tuple combinations, 2D/RGB viz from a (c, y, x) load, warn-and-drop on bad shape / raising viz, contract checks still raise). `test_qc_overlay.py`'s one direct reference to the renamed mask function updated (kept runnable); other bitsnpieces one-offs left as-is per convention. Previous: **detector (BYOD) refactor — `detect()` is now a minimal composer (WIP, see TODO #18)** (no microscope): `detection.detect(load_fun, detector_fun, relabel='distance', clear_border=True, stim_mask_fun=None)` returns a `survey_file -> (labels[, stim_mask])` callable — the experiment-specific parts (which channel(s) to load, which model, which areas are FRAP-eligible) are callables passed in, and `detect()` only applies the stable housekeeping (`clear_border` + gap-free renumber, `relabel_by_distance`/`shuffle_labels`) and contract checks (2D int labels matching the image's (y, x); 2D mask; the one-region-per-cell warning). `stim_mask_fun(labels, image)` has a fixed 2-arg signature (e.g. FRAP an organelle detected in the image within each cell); `default_stimulation_mask` (left half) is no longer applied automatically — it is a `stim_mask_fun` building block (adapter `autofrap._default_stim_mask` drops the image arg). The `detector='dummy'|'cellpose-remote'` name dispatch is gone: `autofrap()`'s default and the CLI now pass `partial(detection.remote_detect_objects, server_url=...)` / `detection.dummy_detect_objects` straight in, and `load_fun` is `partial(nd2_helpers.read_channel, channel=...)` — multi-channel loading (e.g. detect in ch0, filter by a ch1 marker) is a `np.stack` lambda in `load_fun`. A detector that also returns a viz bypasses `detect()` and passes a fully custom `survey_file -> (labels[, mask[, viz]])` to `autofrap()` (module docstring documents both tiers + examples). Breaking for the one-off `test_autofrap_errors.py` (left as-is per convention). Tested in-container with stub `load_fun`s (2D + (y, x, c), all contract checks). Previous: **output naming** (`file_prefix`, flat run dir by default, `_cycle<NN>_` tags — see TODO #8 tail) and **TODO #8 done** (no microscope): per-cycle QC overlay hooked into `autofrap()` — `<stamp>_cNN_survey_qc.png` saved before each stimulation run, warn-and-continue; `detection_fun` return is now a 1–3-tuple `(labels[, stimulation_mask[, viz]])` (no mask → whole-cell FRAP; no viz → blank black canvas in `qc.save_qc_overlay(image=None, ...)`), see TODO #8. **sprintf revert**: `get_optical_confs` back to the documented `"i"` form (TODO #17: live re-check). Previous: **ND Acquisition dialog settings are queryable — TODO #14 first step done** (at microscope): the dialog's *current experiment definition* can be queried read-only with **no document open** (settings persist across NIS restarts): tab active states via `ND_IsAcqTabChecked` (names, case sensitive: **Time, XY, Z, Lambda, Large Image** — the multichannel tab displays as λ; verified with all tabs ticked → all TRUE), loop params via `ND_GetTimeLapsePhaseCount` / `ND_GetTimePhaseSchedule` / `ND_MP_GetCount` / `ND_GetZSeriesExp` (params persist even when the tab is unticked — tab state and params are separate questions). `ND_GetExperimentLoopSize` is **not** usable (queries the open *document*; -9 with none open). New in `nis_util.py`: `get_nd_acq_tabs` (verified) + `_nis_running` guard in `_run_macro` — **`nis_ar -mw` spawns a fresh NIS instance when none is running, which closes again after the macro** (an interrupted probe spawned exactly such a throwaway instance); `_run_macro` now refuses to do that and raises instead. **CAUTION**: `ND_GetLambdaChannel(0, ...)` **crashed NIS Elements** (twice; a `done` marker written after the call survived once — cause not identified, user: dialog settings persistent, 2 channels defined); channel count/names stay out of the pre-flight for now. Macro-language gotcha: `sprintf(buf, fmt, args)` is **not** C-variadic — `args` is a comma-separated string of variable names to substitute; literal strings need `strcpy()`. Probe: `autofrap_bitsnpieces/test_nd_exp_getter_live.py`. Next: integrate the tab check into the `autofrap()` pre-flight (TODO #14). Before that: **QC overlay renderer `autofrap/qc.py` (TODO #8, part 1 —
...
18. **Flexible detectors / bring-your-own (BYOD)** — **WIP (20260905)**:
    ...
    just callables. **Done**: first-class viz hook
    `visualization_fun(image)` (see top entry) — a viz no longer
    requires bypassing `detect()` with a fully custom
    `detection_fun`. **Remaining**: how user-supplied functions are
    actually integrated in practice (notebook-only vs. CLI, sharing
    parts, ...) needs a bit more thought.
...
20. **Default detector composition in `autofrap()`**: as single-FOV
    `autofrap()` is usually called from the multi-position wrapper, don't
    create the default detector inside `autofrap()` — compose it outside
    (fine for testing now; remove for the final version). Note: the CLI
    already composes its own, so the in-function default only serves direct
    notebook calls — extracting the composition into one named helper
    (used by CLI + default) would kill the duplication either way.
...
24. **Survey document already open after the ND run**: `ND_RunExperiment(1)`
    ...
    no-op costing a macro round-trip per cycle; while it runs, the
    following `get_current_document` check verifies nothing. Drop the
    `open_image`, keep the check (it becomes a real verification). Live
    check at the scope.
...
26. **`settle_s` after `set_position` may be redundant**: `StgMoveXY` is
    ...
    (blocking)", XY round-trip at target immediately after the macro
    returned). Live test: time `set_position` on a long move (wall time
    scaling with distance ⇒ blocking), then drop the default to a small
    residual (keep the parameter: vibration margin, return-to-start).
...
29. **One-stimulation-region-per-label helpers** (user TODO): e.g.
    `largest_region_per_label(labels, mask)` / most-central variant in
    `mask_utils`; QC argument: today a contract violation warns, then
    `mask_to_polygon` silently uses the largest region while the overlay
    draws the whole mask (over-promising). Composable via `stim_mask_fun`,
    **not** auto-applied in `build_detector` (the warning is a
    detector-debugging signal).
